...
- Optional **LED window** mode: each artwork shape is placed on F.Mask, B.Mask or both, plus a copper **keep-out zone** with the same outline, so an LED behind the board can shine through — or as a **touch pad** (F.Cu + F.Mask + keep-out)
...
3. Optionally tick **LED window / touch pad** and pick F.Mask + keep-out, B.Mask + keep-out, F.Mask + B.Mask + keep-out, or Touch pad (F.Cu + F.Mask + keep-out) (this replaces the artwork layer — see [LED window](#led-window) below). The keep-out zones show in the KiCad preview as hatched blue areas, as in KiCad.
...
To export LED windows (`front` = F.Mask, `back` = B.Mask, `both` = F.Mask + B.Mask, each plus a copper keep-out) or touch pads (`touch` = F.Cu + F.Mask + keep-out):
...
- as a filled `gr_poly` on the chosen layer(s) — F.Mask, B.Mask, both, or F.Cu + F.Mask for a touch pad, and
- as a keep-out rule area (`zone` with `keepout`) on **F.Cu + B.Cu** (plus the chosen mask layers), with tracks, vias, pads and copper pour not allowed and footprints allowed — so no copper blocks the light. For a touch pad, the keep-out stops tracks and ground pour from running through or under the exposed copper pad (the pad's own F.Cu shape is a graphic, not a track or pour, so it passes DRC); connect the pad to its sense trace yourself in KiCad.
