...
- `ui.js` owns all DOM wiring: drag-and-drop + file input, the layer `<select>` (a "common layers" `<optgroup>` always visible, a "more layers" `<optgroup>` behind a disclosure toggle — the full list mirrors `HEADER`'s layer table), the scale `<input type="number">` (resets to `1` on every new file), stats display, and the copy button.
